...
router.put('/:id', authorize('super_admin','admin'), async (req, res, next) => {
  try {
    const cat = await Category.findOne({ where: { id: req.params.id, company_id: req.companyId } });
...
router.delete('/:id', authorize('super_admin','admin'), async (req, res, next) => {
  try {
    await Category.update({ active: false }, { where: { id: req.params.id, company_id: req.companyId } });
